...
struct StageBlocks {
	...
	inline void* operator[] (int i) {
		...
		return reinterp__ret_cast<void**>(&header) + i;
	}
};
...
namespace Game {
	...
	extern fx32 liquidPos[2];

	extern u32 activatorIDBitmask[2];
	...
	// 02020608
	GEN_SFUNC( Player* getPlayer, s32 playerNo )

	// 02020618
	GEN_SFUNC( Player* setPlayer, s32 playerNo, Player* player)
...
/*
	...
	*/
	...
	// 02020240
	GEN_SFUNC(u8 getPlayerInventoryPowerup, s32 playerNo)

	// 02020250
	GEN_SFUNC(u8 setPlayerInventoryPowerup, s32 playerNo, u8 powerup)

	// 02020220
	GEN_SFUNC(u8 getPlayerDead, s32 playerNo)

	// 02020230
	GEN_SFUNC(u8 setPlayerDead, s32 playerNo, bool isDead)

	// 020200B4
	GEN_SFUNC(u8 getPlayerRespawnState, s32 playerNo)

	// 020200C4
	GEN_SFUNC(u8 setPlayerRespawnState, s32 playerNo, s32 state)

	inline void setVisiblePlain(int plane) {
		Game::visiblePlaneMain = plane;
	}
}
...
enum CollisionFlagID : u8
{
	FLG_NONE,			// none
	FLG_FIREBALL,		// fireball
	FLG_2,
	FLG_3,
	FLG_4,
	FLG_SLIDING,		// player (when sliding)
	FLG_6,
	FLG_7,
	FLG_8,
	FLG_9,
	FLG_DAMAGE_ENTITY,	// snowball, spiked ball
	FLG_11,
	FLG_12,
	FLG_13,
	FLG_14,
...
#define MAKE_FLAG_MASK(...) static_cast<u16>(~__mask_or(FLG_NONE, __VA_ARGS__))
