...
namespace SND
{
	...
	NTR_INLINE void pushCommand(SNDCommandID c, T0 a0, T1 a1 = T1(), T2 a2 = T2(), T3 a3 = T3()) {
		PushCommand_impl(c, u32(a0), u32(a1), u32(a2), u32(a3));
	}

	NTR_INLINE void setMasterVolume(int volume) {
		pushCommand(SND_COMMAND_MASTER_VOLUME, volume);
	}

	NTR_INLINE void setMasterPan(int pan) {
		pushCommand(SND_COMMAND_MASTER_PAN, pan);
	}

	NTR_INLINE void resetMasterPan() {
		setMasterPan(-1);
	}

	NTR_INLINE void setupChannelPcm(int chNo, SNDWaveFormat format, const void* dataAddr, SNDChannelLoop loop, int loopStart, int dataLen, int volume, SNDChannelDataShift shift, int timer, int pan) {
		SND_SetupChannelPcm(chNo, format, dataAddr, loop, loopStart, dataLen, volume, shift, timer, pan);
	}

	NTR_INLINE void setupChannelPsg(int chNo, SNDDuty duty, int volume, SNDChannelDataShift shift, int timer, int pan) {
		pushCommand(SND_COMMAND_SETUP_CHANNEL_PSG, chNo, volume | shift << 8, pan | timer << 8, duty);
	}

	NTR_INLINE void setupChannelNoise(int chNo, int volume, SNDChannelDataShift shift, int timer, int pan) {
		pushCommand(SND_COMMAND_SETUP_CHANNEL_NOISE, chNo, volume | shift << 8, pan | timer << 8);
	}

	NTR_INLINE void setChannelVolume(u32 chBitMask, int volume, SNDChannelDataShift shift) {
		pushCommand(SND_COMMAND_CHANNEL_VOLUME, chBitMask, volume, shift, 0);
	}

	NTR_INLINE void setChannelTimer(u32 chBitMask, int timer) {
		pushCommand(SND_COMMAND_CHANNEL_TIMER, chBitMask, timer, 0, 0);
	}

	NTR_INLINE void setChannelPan(u32 chBitMask, int pan) {
		pushCommand(SND_COMMAND_CHANNEL_PAN, chBitMask, pan, 0, 0);
	}

	NTR_INLINE void startTimer(u32 chBitMask, u32 capBitMask, u32 alarmBitMask, u32 flags) {
		SND_StartTimer(chBitMask, capBitMask, alarmBitMask, flags);
	}

	NTR_INLINE void stopTimer(u32 chBitMask, u32 capBitMask, u32 alarmBitMask, u32 flags) {
		SND_StopTimer(chBitMask, capBitMask, alarmBitMask, flags);
	}

	NTR_INLINE void setupAlarm(int alarmNo, u32 tick, u32 period, SNDAlarmHandler handler, void* arg) {
		SND_SetupAlarm(alarmNo, tick, period, handler, arg);
	}

	NTR_INLINE void setupCapture(SNDCapture capture, SNDCaptureFormat format, void* buffer_p, u32 length, BOOL loopFlag, SNDCaptureIn in, SNDCaptureOut out) {
